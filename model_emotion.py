...
class Model:
	...
	def __init__(self, output_shape):
		...
		# image must size 48, 48, 1
		self.X = tf.placeholder('float', [None, settings.image_size, settings.image_size, 1])
		...
		with tf.name_scope("conv3-64-1"):
			conv6 = tf.nn.relu(conv_layer(pooling3, 3, 64, '64-1'))
			
		with tf.name_scope("maxpool-4"):
			pooling4 = pooling(conv6, stride = 2)
			
		with tf.name_scope("fc-512-1"):
			pooling4 = tf.reshape(pooling4, [-1, 3 * 3 * 64])
			fc1 = tf.nn.relu(fully_connected(pooling4, 512, '512_1'))
